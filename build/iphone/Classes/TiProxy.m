...
//Rare exceptions to indicate a bug in the _camera code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_camera = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_camera]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.camera.module"
