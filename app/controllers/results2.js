 ...
 */
 ...
 function listItemClicked(_event) {
	...
	alert("clicked on " + userInfo.fields.item_name + " " + userInfo.fields.nf_calories);
	var data = userInfo.fields.nf_calories;
	Ti.App.fireEvent('myCustomEvent', {
    myCustomEventValue:  data
});
             w.open();
            $.results2.close();
}
...
Ti.App.addEventListener('myCustomEvent', function(event) {
  var info = event.myCustomEventValue;
